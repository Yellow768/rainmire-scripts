...
function createPerkGui(e, editable, init) {
    ...
    for (var i = 0; i < selected_bad_perk_array.length; i++) {
        ...
        var texture = perkGUI.addTexturedRect(900 + i, "iob:textures/customgui/perks/bad/" + selected_bad_perk_array[i].id + ".png", base_x + 142, 10 + base_y + (36 * i), 256, 256)
        texture.setScale(.10)
    }
    for (var i = 0; i < collected_bad_perk_array.length; i++) {
        player.message(collected_bad_perk_array[i].id + ".png")
        ...
        var texture = perkGUI.addTexturedRect(700 + i, "iob:textures/customgui/perks/bad/" + collected_bad_perk_array[i].id + ".png", base_x + 185, base_y + (27 * i), 256, 256)
        texture.setScale(.10)
    }
...
function perkGuiButton(e) {
    if (e.player.getGui() != perkGUI) { return }
