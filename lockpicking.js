        ...
        if (e.target.storeddata.get("LockDifficulty") != undefined && e.target.storeddata.get("LockDifficulty") > 0) {
            ...
            switch (e.player.getMainhandItem().getDisplayName()) {
                ...
                default:
                    ...
                    executeCommand("/playsound minecraft:entity.shulker.close player @a[distance=..5] ~ ~ ~")
...
function pickLock(e, lockDamage, customModelData) {
    ...
    if (lockDamage > 0) {
        ...
        executeCommand("/particle item minecraft:tripwire_hook{CustomModelData:" + customModelData + "} ~ ~1 ~ .5 .2 .5 .4 10")
        executeCommand("/playsound minecraft:entity.item_frame.rotate_item player @a[distance=..5] ~ ~ ~")
        ...
        if (e.target.storeddata.get("LockDifficulty") > 0) {
            ...
            executeCommand("/playsound iob:ui.lockpick player @a[distance=..5] ~ ~ ~")
        }
        else {
            executeCommand('/title ' + e.player.name + ' actionbar {"text":"Lock Picked!","bold":true,"color":"green"}')
            executeCommand('data merge block ' + e.target.x + ' ' + e.target.y + ' ' + e.target.z + ' {Lock:""}')
            executeCommand("/playsound iob:ui.trap_disabled player @a[distance=..5] ~ ~ ~")
        }
    }
}
