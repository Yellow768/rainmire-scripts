...
function tick(e) {
	if (!isDisabled() && !inGracePeriod) {
		...
		for (var i = 0; i < nE.length; i++) {
			if (getScore(nE[i].name, "Mind") > dataGet("percCheck") - 1) {
				setDisabled(1)
				e.block.executeCommand('/tellraw @a[distance=..15] {"text":"' + nE[i].name + ' disabled a trap! (Mind Level > ' + dataGet("percCheck") + ')","color":"green"}')
