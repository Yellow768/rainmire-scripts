...
function openLineEditor(e, category) {
    ...
    for (var i = 0; i < 8; i++) {
        ...
        LINE_EDITOR.getComponent(100 + i).setText(npc.storeddata.get(lineCategory + "Line" + i))

        //Sounds
        LINE_EDITOR.addTextField(120 + i, 266, 10 + (i * 25), 240, 20)
        LINE_EDITOR.getComponent(120 + i).setText(npc.storeddata.get(lineCategory + "Sound" + i))
...
function customGuiButton(e) {
    switch (e.buttonId) {
        ...
        case 40:
            ...
            npc.storeddata.put("randomLines", randomLines)
...
function customGuiClosed(e) {
    if (e.gui.getID() == id("LINE_GUI")) {
        ...
        for (var i = 0; i < 8; i++) {
            npc.storeddata.put(lineCategory + "Line" + i, e.gui.getComponent(100 + i).getText())
            npc.storeddata.put(lineCategory + "Sound" + i, e.gui.getComponent(120 + i).getText())
        }
    }
    updateArrays(e)
}
...
function updateArrays(e) {
    ...
    for (var j = 0; j < 8; j++) {
        if (npc.storeddata.get(lineCategory + "Line" + j) != '') {
            var lineObject = { line: "", sound: "" };
            lineObject.line = npc.storeddata.get(lineCategory + "Line" + j)
            lineObject.sound = npc.storeddata.get(lineCategory + "Sound" + j)
