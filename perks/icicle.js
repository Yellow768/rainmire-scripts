function perk_icicle(e, cost) {
    ...
    e.player.world.setBlock(blockAbovePos, "quark:blue_crystal_cluster")
    ...
    executeCommand("/particle minecraft:dust .2 .7 1 1 " + blockAbovePos.x + " " + blockAbovePos.y + " " + blockAbovePos.z + " .4 .4 .4 .01 10")
...
function findBlockAbove(e) {
    for (var i = 0; i < getScore("Deftness") * 4; i++) {
        ...
        if (name != "minecraft:air" && name != "minecraft:water" && name != "quark:blue_crystal_cluster" && name.indexOf("ladder") == -1) {
            return pos
        }
    }
    return null
...
function breakIcicle(e, pos) {
    ...
    executeCommand("/particle minecraft:block quark:blue_crystal_cluster " + pos.x + " " + pos.up(1).y + " " + pos.z + " .2 .2 .2 1 100")
